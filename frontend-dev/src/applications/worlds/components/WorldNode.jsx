...
export default function WorldNode({ id, data, selected }) {
  ...
  const handleNodePointerDown = event => { if (event.target instanceof Element && event.target.closest(".react-flow__handle")) return; window.dispatchEvent(new CustomEvent("worlds:node-select", { detail: { nodeId: id } })); };
