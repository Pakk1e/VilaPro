...
const GROUPS = [
  ...
  ["Passive", ["Resistor"]],
  ["Semiconductors", ["Diode", "NPN Transistor", "PNP Transistor", "NMOS", "PMOS"]],
];
...
export default function ComponentLibrary() {
  ...
  return <div data-testid="component-library" className="flex h-full min-h-0 flex-col">
    ...
    <div className="min-h-0 flex-1 overflow-y-auto">
      ...
      <section className="px-3 py-3"><div className="mb-2 text-[10px] font-semibold uppercase tracking-[0.12em] text-[#69717b]">Components</div>{GROUPS.map(([group, types])=>{const entries=definitions.filter(([,definition])=>types.includes(definition.type));if(!entries.length)return null;return <div key={group} className="mb-4"><div className="mb-1.5 text-[9px] font-semibold uppercase tracking-[0.12em] text-[#8a929c]">{group}</div><div className="grid grid-cols-2 gap-1">{entries.map(([key,definition])=><button key={key} type="button" onClick={()=>emit("worlds:add-component", key)} className="border border-[#d9dde2] bg-white px-2 py-2 text-left hover:border-[#9ea8b4] hover:bg-[#f8fafc]"><div className="text-[10px] font-medium text-[#26364d]">{definition.type}</div><div className="mt-0.5 text-[8px] text-[#8a929c]">Add</div></button>)}</div></div>})}</section>
    </div>
  </div>;
}
