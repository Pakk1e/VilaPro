    ...
    circuit: {
      nodes: nodes.map((node) => ({
        ...
        data: node.type === "world"
          ? { componentType: node.data?.componentType, definitionKey: node.data?.definitionKey, properties: node.data?.properties ?? {}, ports: node.data?.ports ?? [] }
...
export default function SimulationPanel({ nodes, edges, onSelectComponent, sweepTargets = [] }) {
  ...
  const simulate = async () => {
    ...
    try {
      ...
      const componentResults = Array.isArray(data.components) ? Object.fromEntries(data.components.map((component) => [component.id, component])) : data.components ?? {};
        ...
        <div className="grid gap-4 p-5 xl:grid-cols-[minmax(220px,0.38fr)_minmax(360px,1fr)]">
          <div className="space-y-4">
            ...
            <div className="rounded-xl border border-[#d9dde2] bg-white px-4 py-4">
              ...
              <div className="mt-1 text-xs leading-5 text-[#69717b]">{simulationIsStale ? "The circuit or setup changed. Run the simulation again to refresh the results." : result ? "Results represent the latest completed simulation run." : "Configure the analysis and run the simulation to generate results."}</div>
            </div>
          </div>

          <div className="min-w-0 space-y-4">
            {configurationError && <div role="alert" aria-live="polite" className="rounded-xl border border-[#ead1d1] bg-[#fff8f8] px-4 py-3"><div className="text-[10px] font-semibold uppercase tracking-[0.14em] text-red-600">Simulation setup error</div><div className="mt-1 whitespace-pre-line text-xs leading-5 text-red-700">{configurationError}</div></div>}
            {simulationIsStale && <div role="status" aria-live="polite" className="rounded-xl border border-[#eadfca] bg-[#fffaf0] px-4 py-3"><div className="text-[10px] font-semibold uppercase tracking-[0.14em] text-[#8a6a2f]">Simulation out of date</div><div className="mt-1 text-xs leading-5 text-[#75613c]">The circuit or simulation setup has changed since these results were calculated.</div></div>}
            {error && <div role="alert" aria-live="assertive" className="rounded-xl border border-[#ead1d1] bg-white px-4 py-3"><div className="text-[10px] font-semibold uppercase tracking-[0.14em] text-red-600">Simulation error</div><div className="mt-1 whitespace-pre-line text-xs leading-5 text-red-700">{error}</div></div>}
            {!result && !error && !configurationError && <div className="flex min-h-[360px] items-center justify-center rounded-xl border border-dashed border-[#d9dde2] bg-white px-6 text-center"><div><div className="text-sm font-semibold text-[#17253a]">No simulation results yet</div><div className="mt-2 max-w-md text-xs leading-5 text-[#69717b]">Configure the analysis on the left, then run the simulation. Results, measurements and plots will live in this workspace.</div></div></div>}
            {result && <ResultExplorer result={result} />}
          </div>
        </div>
      </div>
    </section>
