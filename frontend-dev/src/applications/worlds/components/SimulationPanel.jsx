...
export default function SimulationPanel({ nodes, edges, sweepTargets = [] }) {
  ...
  const scheduleLiveStep = (sessionId) => {
    if (!mountedRef.current || !sessionId) return;
    liveTimerRef.current = window.setTimeout(async () => {
      try {
        const snapshot = await liveRequest(`/simulate/live/${sessionId}/step`);
        if (!mountedRef.current) return;
        setLiveSnapshot(snapshot);
        if (snapshot.status === "running") scheduleLiveStep(sessionId);
        else setRunning(false);
      } catch (liveError) {
        if (!mountedRef.current) return;
        setRunning(false);
        setError(liveError?.message ?? "Live simulation step failed");
      }
    }, 500);
  };
  ...
  const startLive = async () => {
    ...
    try {
      ...
      if (snapshot.status === "running") scheduleLiveStep(snapshot.session_id);
    } catch (liveError) {
      if (!mountedRef.current) return;
      setRunning(false);
