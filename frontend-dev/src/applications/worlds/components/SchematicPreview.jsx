...
export default function SchematicPreview({ nodes, edges, selectedNodeId, onSelectComponent, selectedResultEntity, liveSnapshot = null }) {
  ...
  const liveMeasurements = useMemo(() => new Map((schematic?.instances ?? []).map((instance) => [instance.id, getLiveComponentMeasurements(liveSnapshot, instance)])), [liveSnapshot, schematic]);
  const liveActive = liveSnapshot?.status === "running" || liveSnapshot?.status === "paused";
