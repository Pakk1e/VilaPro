...
function getLiveBranchCurrent(signals, instance, suffix = "") {
  ...
  const names = [instance?.name, identifier].filter(Boolean).map(escapeRegExp);
  const componentPattern = new RegExp(`component=['"]?(?:${names.join("|")})['"]?`);
...
function getLiveCurrentArrow(instance, liveMeasurements, schematic) {
  const current = liveMeasurements?.current;
  if (!Number.isFinite(current) || current === 0 || !schematic) return null;
  const p = getSchematicPort(instance, "p", schematic);
  const n = getSchematicPort(instance, "n", schematic);
  if (!p || !n) return null;
  const dx = n.x - p.x;
  const dy = n.y - p.y;
...
function SchematicSymbol({ instance, position, orientation, selected, resultHighlighted, liveMeasurements, liveCurrentArrow, onSelect }) {
  ...
  return <g transform={`translate(${position.x} ${position.y})`} onClick={() => onSelect?.(instance.id)} className="cursor-pointer" role="button" tabIndex={0} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onSelect?.(instance.id); } }}>
    ...
    {liveCurrentArrow && <line x1={liveCurrentArrow.x1 - position.x} y1={liveCurrentArrow.y1 - position.y} x2={liveCurrentArrow.x2 - position.x} y2={liveCurrentArrow.y2 - position.y} stroke={LIVE} strokeWidth="3" strokeLinecap="round" markerEnd="url(#live-current-arrow)" pointerEvents="none" />}
...
export default function SchematicPreview({ nodes, edges, selectedNodeId, onSelectComponent, selectedResultEntity, liveSnapshot = null }) {
  ...
  useEffect(() => {
    const handleLiveSnapshot = (event) => setLiveEventSnapshot(event.detail ?? null);
    window.addEventListener("worlds:live-snapshot", handleLiveSnapshot);
    return () => window.removeEventListener("worlds:live-snapshot", handleLiveSnapshot);
  }, []);
  ...
  const liveCurrentArrows = useMemo(() => new Map((schematic?.instances ?? []).map((instance) => [instance.id, getLiveCurrentArrow(instance, liveMeasurements.get(instance.id), schematic)])), [liveMeasurements, schematic]);
  ...
  return <div className="flex h-full w-[38%] min-w-0 flex-col overflow-hidden border-r border-[#d9dde2] bg-white">
    <div className="flex shrink-0 items-center justify-between border-b border-[#e4e7eb] px-4 py-3">
      <div><div className="text-[10px] font-semibold uppercase tracking-[0.14em] text-[#58718f]">Circuit schematic</div><div className="mt-0.5 text-[10px] text-[#69717b]">{liveActive ? "Live measurements shown on components" : "Select a component to inspect it"}</div></div>
      <div className="text-[10px] text-[#8a929c]">{schematic?.instances.length ?? 0} components</div>
    </div>
    {liveActive && <div className="flex shrink-0 items-center gap-2 border-b border-[#d7e8df] bg-[#f3faf6] px-4 py-2"><span className="h-2 w-2 shrink-0 rounded-full bg-[#2f7d5a]" /><div className="min-w-0 truncate text-[10px] font-medium text-[#2f624c]">Live circuit measurements</div><div className="ml-auto text-[10px] tabular-nums text-[#49725f]">t = {Number.isFinite(Number(effectiveLiveSnapshot?.independent_value)) ? Number(effectiveLiveSnapshot.independent_value).toExponential(3) : "—"} s</div></div>}
    {liveActive && <div className="flex shrink-0 items-center gap-2 border-b border-[#d7e8df] bg-white px-4 py-1.5 text-[9px] text-[#49725f]"><span className="inline-flex items-center gap-1"><span className="h-px w-4 bg-[#2f7d5a]" aria-hidden="true" /><span>Arrow = conventional current direction</span></span><span className="ml-auto">positive current: p → n</span></div>}
    ...
    <div className="min-h-0 flex-1 overflow-auto bg-[#fbfbfa] p-3">
      {schematic?.error ? <div className="flex h-full min-h-[360px] items-center justify-center px-8 text-center"><div><div className="text-sm font-semibold text-[#17253a]">Schematic unavailable</div><div className="mt-1 text-xs leading-5 text-[#69717b]">{schematic.error}</div></div></div> : <svg viewBox={`${schematic.bounds.minX} ${schematic.bounds.minY} ${schematic.bounds.width} ${schematic.bounds.height}`} className="h-full min-h-[360px] w-full min-w-0" role="img" aria-label="Circuit schematic preview" preserveAspectRatio="xMidYMid meet">
        <defs><pattern id="schematic-grid" width="24" height="24" patternUnits="userSpaceOnUse"><circle cx="2" cy="2" r="1" fill="#e1e4e7" /></pattern><marker id="live-current-arrow" markerWidth="7" markerHeight="7" refX="6" refY="3.5" orient="auto" markerUnits="strokeWidth"><path d="M 0 0 L 7 3.5 L 0 7 z" fill={LIVE} /></marker></defs>
