  ...
  if (points.length < 2) {
    return (
      <div className="flex h-56 items-center justify-center rounded-lg border border-dashed border-[#d9dde2] bg-[#fafbfc] px-4 text-center text-xs text-[#69717b]">
        At least two valid result points are required to plot the response.
      </div>
    );
  }
    ...
    <div className="overflow-hidden rounded-lg border border-[#e4e7eb] bg-white">
      <div className="flex items-center justify-between border-b border-[#e4e7eb] px-4 py-3">
        <div>
          ...
          <div className="mt-1 text-xs text-[#8a929c]">{series?.label ?? "Response"} versus {xLabel}</div>
        </div>
      ...
      <div className="overflow-x-auto px-3 py-3">
        <svg viewBox={`0 0 ${width} ${height}`} className="h-auto min-w-[560px] w-full cursor-crosshair" role="img" aria-label={`${series?.label ?? "Response"} versus ${xLabel} result plot`} onClick={handleChartClick}>
          ...
          {xTicks.map((tick) => {
            const x = scaleX(tick);
            return <g key={`x-${tick}`}><line x1={x} x2={x} y1={margin.top} y2={height - margin.bottom} stroke="#f0f1f3" strokeWidth="1" /><text x={x} y={height - margin.bottom + 19} textAnchor="middle" fontSize="10" fill="#69717b">{formatTick(tick)}</text></g>;
          })}
          ...
          {failedRows.map((row, index) => {
            ...
            return <g key={`failed-${row.sweepValue}-${index}`} className="text-red-700"><title>{row.error ?? "Result point failed"}</title><line x1={x - 4} x2={x + 4} y1={y - 4} y2={y + 4} stroke="currentColor" strokeWidth="1.5" /><line x1={x - 4} x2={x + 4} y1={y + 4} y2={y - 4} stroke="currentColor" strokeWidth="1.5" /></g>;
          })}
