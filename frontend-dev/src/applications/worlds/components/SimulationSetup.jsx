...
export default function SimulationSetup({ config, onChange, sweepTargets = [], voltageSources = [] }) {
  ...
  const changeSweepTarget = (source) => {
    ...
    onChange({ settings: { source, parameter: target?.parameters?.[0]?.parameter ?? (typeof target?.parameters?.[0] === "string" ? target.parameters[0] : "") } });
  };
    ...
    <div className="px-4 py-4">
      ...
      {config.analysis === SIMULATION_ANALYSES.DC_SWEEP && (
        <div className="mt-4 space-y-3">
          ...
          {selectedTarget && selectedTarget.parameters.length > 1 && <div><label className="text-[10px] font-medium text-[#69717b]" htmlFor="sweep-parameter">Parameter</label><select id="sweep-parameter" value={config.settings?.parameter ?? ""} onChange={(event) => changeSweepSetting("parameter", event.target.value)} className="mt-1 w-full rounded-md border border-[#d9dde2] bg-white px-2.5 py-2 text-xs text-[#26364d] outline-none focus:border-[#58718f]"><option value="">Select a parameter...</option>{selectedTarget.parameters.map((item) => { const parameter = typeof item === "string" ? item : item.parameter; const label = typeof item === "string" ? item : item.label ?? item.parameter; return <option key={parameter} value={parameter}>{label}{item?.unit ? ` (${item.unit})` : ""}</option>; })}</select></div>}
