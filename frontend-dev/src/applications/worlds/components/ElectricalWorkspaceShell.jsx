...
function GridIcon() { return <svg viewBox="0 0 16 16" className="h-3.5 w-3.5" fill="none" stroke="currentColor" strokeWidth="1.15"><path d="M3 3h10v10H3zM6.33 3v10M9.67 3v10M3 6.33h10M3 9.67h10"/></svg>; }
...
export default function ElectricalWorkspaceShell({ children, library, inspector, instrument, headerCenter }) {
  ...
  return <div data-testid="electrical-workspace" data-focus-mode={focusMode ? "true" : "false"} className="flex h-full min-h-0 flex-col overflow-hidden bg-[#eef1f3] text-[#17253a]">
    ...
    <div className="relative min-h-0 flex-1 overflow-hidden">
      ...
      {!focusMode && <div className="absolute bottom-4 left-4 z-20 flex items-center gap-0.5 rounded-lg border border-[#d1d8df] bg-white/94 p-1 shadow-[0_6px_18px_rgba(24,37,58,0.09)] backdrop-blur"><button type="button" aria-label="Toggle grid" className="flex h-7 w-7 items-center justify-center rounded-md text-[#718094] hover:bg-[#eef2f4] hover:text-[#30445b]"><GridIcon /></button><div className="mx-1 h-4 w-px bg-[#e0e4e8]"/><span className="px-1.5 text-[8px] font-medium uppercase tracking-[0.1em] text-[#98a2ad]">Scroll to zoom · drag to pan</span></div>}
