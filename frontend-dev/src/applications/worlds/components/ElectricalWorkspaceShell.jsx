  ...
  return <div data-testid="electrical-workspace" data-focus-mode={focusMode ? "true" : "false"} className="flex h-full min-h-0 flex-col overflow-hidden bg-[#eef1f4] text-[#17253a]">
    <header className="relative z-50 flex h-12 shrink-0 items-center border-b border-[#d8dde4] bg-[#f8fafb]/95 px-3 shadow-[0_1px_8px_rgba(24,37,58,0.05)] backdrop-blur">
      <div className="flex min-w-0 shrink-0 items-center gap-3">
        <div className="flex items-center gap-2"><div className="flex h-7 w-7 items-center justify-center rounded-md bg-[#17253a] text-[8px] font-bold tracking-[0.08em] text-white">LO</div><div className="text-[10px] font-bold uppercase tracking-[0.16em] text-[#536174]">Lab OS</div></div>
        <div className="h-5 w-px bg-[#d9dee5]" />
        <div className="min-w-0 truncate text-[11px] font-semibold text-[#26364d]">Electrical Workspace</div>
      </div>
      <div data-testid="workspace-header-context" className="min-w-0 flex-1 px-5">{headerCenter}</div>
      <div className="flex shrink-0 items-center gap-0.5 rounded-xl border border-[#dce1e7] bg-[#eef1f4] p-0.5">
        {!focusMode && <><SurfaceToggle label="Library" active={libraryOpen} onClick={() => setLibraryOpen(value => !value)} /><SurfaceToggle label="Inspector" active={inspectorOpen} onClick={() => setInspectorOpen(value => !value)} /><SurfaceToggle label="Instruments" active={instrumentOpen} onClick={() => setInstrumentOpen(value => !value)} /></>}
        <button type="button" onClick={() => setFocusMode(value => !value)} className="inline-flex h-8 items-center rounded-lg bg-white px-3 text-[10px] font-semibold text-[#33445a] shadow-sm transition hover:bg-[#fdfefe]">{focusMode ? "Exit focus" : "Focus"}</button>
      </div>
    </header>
    ...
    <div className="relative min-h-0 flex-1 overflow-hidden p-2">
      ...
      {!focusMode && instrumentOpen && <div data-testid="instrument-resize-handle" role="separator" aria-label="Resize instrument panel" aria-orientation="horizontal" onPointerDown={beginResize} className="absolute bottom-[305px] left-1/2 z-50 flex h-4 w-20 -translate-x-1/2 cursor-ns-resize items-center justify-center rounded-full border border-[#d0d7df] bg-white/95 shadow-sm"><span className="h-1 w-8 rounded-full bg-[#aeb8c4]" /></div>}
    </div>
  </div>;
}
