...
export default function ElectricalWorkspaceShell({ children, library, inspector, instrument, headerCenter }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("pointermove", onPointerMove); window.addEventListener("pointerup", onPointerUp); window.addEventListener("worlds:selection-change", onSelection); window.addEventListener("worlds:add-component", onComponentAdded); window.addEventListener("worlds:add-probe", onProbeAdded);
  ...
  return <div data-testid="electrical-workspace" data-focus-mode={focusMode ? "true" : "false"} className="flex h-full min-h-0 flex-col overflow-hidden bg-[#eef1f3] text-[#17253a]">
    ...
    <div className="relative min-h-0 flex-1 overflow-hidden">
      ...
      {!focusMode && libraryOpen && <aside data-testid="workspace-library-surface" className="absolute bottom-4 left-4 top-4 z-30 flex w-[292px] flex-col overflow-hidden rounded-xl border border-[#d0d8df] bg-[#fbfcfd]/98 shadow-[0_18px_42px_rgba(24,37,58,0.16)] backdrop-blur">{library}</aside>}
      {!focusMode && inspectorOpen && <aside data-testid="workspace-inspector-surface" className="absolute bottom-4 right-4 top-4 z-30 flex w-[300px] flex-col overflow-hidden rounded-xl border border-[#d0d8df] bg-[#fbfcfd]/98 shadow-[0_18px_42px_rgba(24,37,58,0.16)] backdrop-blur">{inspector}</aside>}
