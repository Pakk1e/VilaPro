...
export function getOperatingPointResponseSeries(result) {
  ...
  for (const [id, component] of Object.entries(result?.components ?? {})) {
    const context = getCircuitComponent(result, id); const name = context?.name ?? component?.name ?? id; const base = { entityType: "component", entityId: id, contextTitle: name, ports: context?.ports ?? {} };
