...
test("dc sweep does not expose component-parameter targets", () => {
  assert.equal(getDcSweepValidationError({ ...DEFAULT_DC_SWEEP_SETTINGS, source: "R1-id", parameter: "R", start: 50, stop: 150, step: 50 }, targets), "The selected sweep source is no longer available.");
});

test("dc sweep rejects an unavailable parameter", () => {
  assert.equal(getDcSweepValidationError({ ...DEFAULT_DC_SWEEP_SETTINGS, source: "V1-id", parameter: "I", start: 0, stop: 10, step: 1 }, targets), "The selected sweep source parameter is no longer available.");
});

test("dc sweep rejects a missing source", () => {
  assert.equal(getDcSweepValidationError({ start: 0, stop: 10, step: 1 }, ["V1"]), "Select a voltage source to sweep.");
});

test("dc sweep rejects an unavailable source", () => {
  assert.equal(getDcSweepValidationError({ source: "V2", start: 0, stop: 10, step: 1 }, ["V1"]), "The selected sweep source is no longer available.");
});

test("dc sweep validates step direction and zero step", () => {
  assert.equal(getDcSweepValidationError({ source: "V1", start: 0, stop: 10, step: 0 }, ["V1"]), "Step cannot be zero.");
  assert.equal(getDcSweepValidationError({ source: "V1", start: 0, stop: 10, step: -1 }, ["V1"]), "Step must be positive when start is below stop.");
  assert.equal(getDcSweepValidationError({ source: "V1", start: 10, stop: 0, step: 1 }, ["V1"]), "Step must be negative when start is above stop.");
});
