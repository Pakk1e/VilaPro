...
test("selected component exposes object-local inspection without opening a dashboard panel", async ({ page }) => {
  ...
  await expect(page.getByTestId("object-local-inspector")).toBeVisible();
  await expect(page.getByTestId("object-local-inspector")).toContainText("Resistor 1");
  await expect(page.getByTestId("object-local-inspector")).toContainText("Resistance");
  await expect(page.getByTestId("object-local-inspector").getByRole("spinbutton", { name: "Object Resistance" })).toHaveValue("100");
  await expect(page.getByTestId("workspace-inspector-surface")).toBeVisible();
  await page.getByTestId("object-local-inspector").getByRole("spinbutton", { name: "Object Resistance" }).fill("470");
  ...
  await page.getByTestId("object-local-inspector").getByRole("button", { name: "Current" }).click();
