...
test("diode example renders and solves forward conduction", async ({ page }) => {
  ...
  await expect(page.getByRole("img", { name: "Circuit schematic preview" }).locator("text").filter({ hasText: "Diode 1" })).toHaveCount(1);
  ...
  await expect(row).toContainText(/704\.3\s*mV/);
  await expect(row).toContainText(/4\.3\s*mA/);
