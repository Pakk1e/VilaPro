...
class WorldSemanticAnalyzer:
    ...
    def _resolve_quantity(self, name: str):
        ...
        if quantity is None:
            raise WorldSemanticError(
                f"Unknown quantity or dimension: {name}"
            )
        ...
        if name in self._resolving:
            ...
            cycle = (
                self._resolving[cycle_start:]
                + [name]
            )

            raise WorldSemanticError(
                "Circular quantity dependency: "
                + " -> ".join(cycle)
            )
        ...
        try:
            ...
            if quantity.unit is not None:
                try:
                    unit = self.units.require(
                        quantity.unit
                    )

                except UnitError as error:
                    raise WorldSemanticError(
                        f"Cannot resolve unit of "
                        f"{name}: {error}"
                    ) from error
                ...
                if unit.dimension != dimension:
                    raise WorldSemanticError(
                        ...
                        f"unit {unit.symbol!r} has dimension "
                        f"{unit.dimension}, expected {dimension}"
                    )

            # ----------------------------------------
            # Register resolved quantity
            # ----------------------------------------

            self.dimensions.define(
                name,
                dimension,
            )
    ...
    def _resolve_expression(self, expression):
        """
        Replace quantity references with their resolved
        dimensions.

        The resulting expression contains only SI dimension
        ...
        if isinstance(expression, Variable):
            dimension = self._resolve_quantity(
                expression.name
            )
        ...
        if isinstance(expression, Unary):
            return Unary(
                ...
                operand=self._resolve_expression(
                    expression.operand
                ),
            )
        ...
        if isinstance(expression, Binary):
            return Binary(
                left=self._resolve_expression(
                    expression.left
                ),
                operator=expression.operator,
                right=self._resolve_expression(
                    expression.right
                ),
            )
    ...
    def _register_physical_functions(self):
        port_types = {
            ...
            if (
                ...
                and symbol.port_type is not None
            )
        }
        ...
        self.functions = create_world_physical_function_registry(
            ...
            factories=(
                create_electrical_world_registry,
            ),
        )
