...
class CapacitorTransientModel:
    ...
    def capacitance(component: SimulationComponent) -> float:
        ...
        if value is None:
            raise DynamicComponentError(
                f"Capacitor '{component.name}' must define parameter 'C'"
            )
        try:
            capacitance = float(value)
        except (TypeError, ValueError):
            raise DynamicComponentError(
                f"Capacitor '{component.name}' capacitance must be a finite number"
            ) from None
        if not capacitance > 0:
            raise DynamicComponentError(
                f"Capacitor '{component.name}' capacitance must be greater than zero"
            )
        return capacitance
    ...
    def initial_voltage(component: SimulationComponent) -> float:
        value = component.parameters.get(
            "initial_voltage",
            component.parameters.get("initialVoltage", 0.0),
        )
        try:
            voltage = float(value)
        except (TypeError, ValueError):
            raise DynamicComponentError(
                f"Capacitor '{component.name}' initial voltage must be a finite number"
            ) from None
    ...
    def prepare_equation(
        ...
        if "p" not in component.ports or "n" not in component.ports:
            raise DynamicComponentError(
                f"Capacitor '{component.name}' must define p/n ports"
            )
        if dt is None:
            # At t=start, enforce the configured initial capacitor voltage.
            return Equation(
                left=FunctionCall(
                    "voltage",
    ...
    def prepare_step(
        ...
        for component in model.components:
            ...
            previous_voltage = previous_state.get(
                ...
                self.model.initial_voltage(component),
            )
            if not isinstance(previous_voltage, (int, float)) or isinstance(previous_voltage, bool):
                raise DynamicComponentError(
                    f"Capacitor '{component.name}' state voltage must be numeric"
                )
            equation = self.model.prepare_equation(
                component,
    ...
    def accept_step(
        ...
        for component in getattr(result, "instances", {}).values():
            ...
            try:
                ...
                raise DynamicComponentError(
                    f"Unable to read transient voltage for capacitor '{component.name}'"
                ) from exc
