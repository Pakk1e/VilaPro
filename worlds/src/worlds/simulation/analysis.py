...
class DCOperatingPointAnalysis:
    ...
    def run(self, model: SimulationModel, *, known: dict[object, float] | None = None, configuration: SimulationConfiguration | None = None, session: SimulationSession | None = None) -> SimulationResult:
...
class DCSweepAnalysis:
    key = DC_SWEEP
    def run(self, model: SimulationModel, *, known: dict[object, float] | None = None, configuration: SimulationConfiguration | None = None, session: SimulationSession | None = None) -> DCSweepResult:
        ...
        for point in points:
            ...
            try:
                ...
                continue
            result = SimulationResult(values=solved.values, instances={component.name: component for component in swept_model.components})
            results.append(result)
            errors.append(None)
            if session is not None:
                session.record_point(result, time=float(point))
        return DCSweepResult(source_id=source_id, parameter=parameter, points=tuple(points), results=tuple(results), errors=tuple(errors))
    ...
    def _parse_settings(settings: Mapping[str, object], model: SimulationModel):
        ...
        if expected_parameter is None:
            raise SimulationAnalysisError(f"dc_sweep source '{source_id}' is not an independent voltage or current source")
        if parameter is None:
            parameter = expected_parameter
        if parameter != expected_parameter:
            raise SimulationAnalysisError(f"dc_sweep source '{source_id}' must be swept using parameter '{expected_parameter}'")
        if parameter not in source.parameters:
            raise SimulationAnalysisError(f"dc_sweep source '{source_id}' does not expose parameter '{parameter}'")
...
def _override_component_parameter(model: SimulationModel, *, source_id: str, parameter: str, value: float) -> SimulationModel:
    components = []
    found = False
    ...
    if not found:
        raise SimulationAnalysisError(f"dc_sweep source '{source_id}' does not exist in the circuit")
    # Preserve the original node set. Replacing the component list with a fresh
    # SimulationModel must not discard topology metadata used to build KCL.
    return SimulationModel(components=components, nodes=set(model.nodes))


_ANALYSES: dict[str, SimulationAnalysis] = {DC_OPERATING_POINT: DCOperatingPointAnalysis(), DC_SWEEP: DCSweepAnalysis()}
...
def get_simulation_analysis(key: str) -> SimulationAnalysis:
    try:
        ...
        supported = ", ".join(SUPPORTED_ANALYSES)
        raise SimulationAnalysisError(f"Unsupported simulation analysis '{key}'. Supported analyses: {supported}") from None
