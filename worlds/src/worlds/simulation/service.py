        ...
        for name,component in result.instances.items():
            if component.component_type in {"NPNTransistor","PNPTransistor"}:
                ...
                if component.component_type=="NPNTransistor":
                    vbe=result.node_voltage(base)-result.node_voltage(emitter); vce=result.node_voltage(collector)-result.node_voltage(emitter); ib=result.component_current(name,base,emitter); ic=result.component_current(name,collector,emitter); ie=ib+ic; region_voltage=vce; cutoff=ib<=1e-12 or vbe<float(component.parameters.get("Vbe",0))-1e-9; saturation=vce<=float(component.parameters.get("VceSat",0))+1e-9; result_fields={"vbe":vbe,"vce":vce,"baseCurrent":ib,"collectorCurrent":ic,"emitterCurrent":ie}
                else:
                    veb=result.node_voltage(emitter)-result.node_voltage(base); vec=result.node_voltage(emitter)-result.node_voltage(collector); ib=result.component_current(name,emitter,base); ic=result.component_current(name,emitter,collector); ie=ib+ic; region_voltage=vec; cutoff=ib<=1e-12 or veb<float(component.parameters.get("Vbe",0))-1e-9; saturation=vec<=float(component.parameters.get("VceSat",0))+1e-9; result_fields={"veb":veb,"vec":vec,"baseCurrent":ib,"collectorCurrent":ic,"emitterCurrent":ie}
            ...
            if component.component_type in {"NMOS","PMOS"}:
                ports=component.ports; gate,drain,source=ports.get("g"),ports.get("d"),ports.get("s")
                if not gate or not drain or not source: raise SimulationServiceError(f"{component.component_type} '{component.display_name}' must have g/d/s ports")
                vth=float(component.parameters.get("Vth",0)); rds_on=float(component.parameters.get("RdsOn",0)); vgs=result.node_voltage(gate)-result.node_voltage(source); vsg=result.node_voltage(source)-result.node_voltage(gate); vds=result.node_voltage(drain)-result.node_voltage(source); vsd=result.node_voltage(source)-result.node_voltage(drain)
                if component.component_type=="NMOS":
                    channel_current=result.component_current(name,drain,source); region="on" if vgs>=vth-1e-9 else "off"; components.append({"id":component.component_id,"name":component.display_name,"type":component.component_type,"voltage":vds,"current":channel_current,"power":vds*channel_current,"region":region,"vgs":vgs,"vds":vds,"drainCurrent":channel_current,"gateCurrent":result.component_current(name,gate,source)})
                else:
                    channel_current=result.component_current(name,source,drain); region="on" if vsg>=vth-1e-9 else "off"; components.append({"id":component.component_id,"name":component.display_name,"type":component.component_type,"voltage":vsd,"current":channel_current,"power":vsd*channel_current,"region":region,"vsg":vsg,"vsd":vsd,"sourceToDrainCurrent":channel_current,"gateCurrent":result.component_current(name,gate,source)})
                continue
