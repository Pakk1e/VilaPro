...
class SimulationSeriesFactory:
    ...
    def from_result(self, result: SimulationResultModel) -> tuple[SimulationSeries, ...]:
        ...
        for dataset in result.datasets:
            ...
            for key in keys:
                ...
                if len(y) != len(axis):
                    raise SimulationSeriesError(
                        f"Dataset '{dataset.name}' is not aligned with axis '{axis_name}'"
                    )
                series.append(
                    SimulationSeries.from_values(
                        id=f"{dataset.name}:{key}",
                        label=f"{'V' if quantity == 'voltage' else 'I'}({key})",
                        x=axis,
                        y=tuple(self._numeric_or_none(value) for value in y),
                        quantity=quantity,
                        unit=unit,
                        source=f"{source_kind}:{key}",
                    )
                )
        return tuple(series)
    ...
    def _axis_name(result: SimulationResultModel) -> str:
        ...
        if analysis == "dc_sweep":
            return "sweep"
        if result.datasets and result.datasets[0].name in {"time", "sweep"}:
    ...
    def _axis_values(result: SimulationResultModel, axis_name: str) -> tuple[float, ...]:
        ...
        try:
            ...
            raise SimulationSeriesError(f"Axis dataset '{axis_name}' contains a non-numeric value") from None
    ...
    def _keys(rows: list[object] | tuple[object, ...]) -> tuple[str, ...]:
        ...
        for row in rows:
            if isinstance(row, Mapping):
                ...
                raise SimulationSeriesError("Series dataset contains a non-mapping row")
    ...
    def _numeric_or_none(value: object | None) -> float | None:
        ...
        try:
            ...
            raise SimulationSeriesError("Series dataset contains a non-numeric value") from None
