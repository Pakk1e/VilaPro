...
def has_transistors(model: SimulationModel) -> bool:
    return any(component.component_type in {"NPNTransistor", "PNPTransistor"} for component in model.components)


def solve_transistor_network(model: SimulationModel, *, known: dict[object, float] | None = None) -> SimulationResult:
    """Solve NPN and PNP BJTs with deterministic piecewise-linear models.

    NPN: cutoff, VBE = Vbe_on with IC = Beta*IB, or VBE = Vbe_on and VCE = Vce_sat.
    PNP: cutoff, VEB = Vbe_on with IE->C = Beta*IE->B, or VEB = Vbe_on and VEC = Vce_sat.

    This is an educational large-signal DC/PWL model. It intentionally does
    not claim frequency-dependent small-signal BJT behaviour.
    """
    states = {}
    for component in model.components:
        if component.component_type == "NPNTransistor": states[component.name] = NPNRegion.CUTOFF
        elif component.component_type == "PNPTransistor": states[component.name] = PNPRegion.CUTOFF
...
def _apply_transistor_states(model: SimulationModel, states) -> SimulationModel:
    ...
    for component in model.components:
        ...
        if component.component_type == "NPNTransistor":
            ib = FunctionCall("current", (b, e)); ic = FunctionCall("current", (c, e))
            vbe = FunctionCall("voltage", (b, e)); vce = FunctionCall("voltage", (c, e))
            ...
            ib = FunctionCall("current", (e, b)); ic = FunctionCall("current", (e, c))
            veb = FunctionCall("voltage", (e, b)); vec = FunctionCall("voltage", (e, c))
...
def _next_npn_state(component, state, result):
    ...
    if not base or not collector or not emitter: raise SolverError(f"NPN transistor '{component.component_id}' must have b/c/e ports")
    vbe_on = _positive_parameter(component, "Vbe", "Vbe must be greater than zero")
    vce_sat = _positive_parameter(component, "VceSat", "VceSat must be greater than zero")
    ...
    if state == NPNRegion.ACTIVE:
        if vbe < vbe_on - TRANSISTOR_VOLTAGE_TOLERANCE or ib < -TRANSISTOR_CURRENT_TOLERANCE: return NPNRegion.CUTOFF
        if vce < vce_sat - TRANSISTOR_VOLTAGE_TOLERANCE: return NPNRegion.SATURATION
        return state
    if vbe < vbe_on - TRANSISTOR_VOLTAGE_TOLERANCE or ib < -TRANSISTOR_CURRENT_TOLERANCE: return NPNRegion.CUTOFF
...
def _next_pnp_state(component, state, result):
    ports = component.ports
    base, collector, emitter = ports.get("b"), ports.get("c"), ports.get("e")
    if not base or not collector or not emitter: raise SolverError(f"PNP transistor '{component.component_id}' must have b/c/e ports")
    vbe_on = _positive_parameter(component, "Vbe", "Vbe must be greater than zero")
    vce_sat = _positive_parameter(component, "VceSat", "VceSat must be greater than zero")
    ...
    if state == PNPRegion.ACTIVE:
        if veb < vbe_on - TRANSISTOR_VOLTAGE_TOLERANCE or ib < -TRANSISTOR_CURRENT_TOLERANCE: return PNPRegion.CUTOFF
        if vec < vce_sat - TRANSISTOR_VOLTAGE_TOLERANCE: return PNPRegion.SATURATION
        return state
    if veb < vbe_on - TRANSISTOR_VOLTAGE_TOLERANCE or ib < -TRANSISTOR_CURRENT_TOLERANCE: return PNPRegion.CUTOFF
...
def _positive_parameter(component, name, message):
    ...
    if value <= 0: raise SolverError(f"{component.component_type} '{component.component_id}': {message}")
    return value
